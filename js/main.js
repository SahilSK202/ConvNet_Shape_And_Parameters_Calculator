function main() {
  ...
  let parameters_el = document.getElementById("n_params");
  ...
  function update_input_output() {
    ...
    var w2 = Math.floor(
      (v.w1.value - v.f.value + 2 * v.p.value) / v.s.value + 1
    );
    var h2 = Math.floor(
      (v.h1.value - v.f.value + 2 * v.p.value) / v.s.value + 1
    );

    let message = "";
    if (w2 < 1 || h2 < 1) {
      message =
        "Spatial extent can't be greater than padded input size. Increase P, increase H<sub>1</sub> / W<sub>1</sub>, or reduce F.";
